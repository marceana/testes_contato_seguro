...
class View {
  ...
  raiseMessage(tbl, type) {
    ...
    if (type === "empty") {
      ...
      div.textContent =
        "Registro não pôde ser editado, verifique se os campos foram preenchidos corretamente.";
      div.setAttribute(
        "class",
      ...
      div.textContent =
        "Registro não pôde ser inserido, verifique se os campos foram preenchidos corretamente.";
      div.setAttribute(
        "class",
        "alert alert-danger alert-dismissible fade show"
      );
    ...
    for (let i in lst) {
      ...
      for (let key in lst[i]) {
        ...
        if (lst[i][key].match(/\d{4}-\d{2}-\d{2}/)) {
          td.textContent = new Date(lst[i][key]).toLocaleDateString("pt-BR", {
            timeZone: "UTC",
          });
        } else {
...
document.getElementById("c-submit").addEventListener(
  "click",
  function () {
...
document.getElementById("u-submit").addEventListener(
  "click",
  function () {
    let table = document.getElementById("table-list");
